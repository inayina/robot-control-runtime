...
- 实现合同与构建/运行命令：[QT_DEVICE_WORKBENCH.md](QT_DEVICE_WORKBENCH.md)
- 面试卡：[KNOWLEDGE_BASE.md](KNOWLEDGE_BASE.md) §6.14 / §10.19
- 开发计划里的 UI 边界：[DEVICE_TEST_DIAGNOSTIC_WORKBENCH_DEVELOPMENT_PLAN.md](DEVICE_TEST_DIAGNOSTIC_WORKBENCH_DEVELOPMENT_PLAN.md)
...
读代码时只问所有权和线程，不要记控件 API。

### 7.1 `main.cpp`：组装，不实现业务
...
### 7.2 `workbench_controller.*`：用例，不画控件
...
### 7.4 `qt_metatypes.hpp`：最小 Qt 污染面
