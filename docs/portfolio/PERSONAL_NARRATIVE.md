...
**部署**:在 Orange Pi 4 Pro 上完成 aarch64 原生构建、release 安装、systemd unit、CPU affinity/governor 控制,并跑出双平台 12 格调度矩阵(OTHER/FIFO × idle/stress × 1/5/10ms)。stock 默认无 CAN，`rcrd` 未冷启动常驻；可选 can1 已跑过 `vcan0 + rcrd` 软件链。另有独立 can2/MCP2515 ↔ STM32F103 dirty-tree 双向 CAN V1、PC13 输出、SG90 无负载双位置目视动作和专用仲裁诊断，但尚未运行 `rcrd --can can0`、Qt physical Health 或完整故障矩阵。
