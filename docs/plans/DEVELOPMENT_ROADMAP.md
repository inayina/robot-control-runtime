...
不推进 Workbench A2，也不为发布绿灯再改内核。EtherCAT、物理 CAN、Modbus 扩展和
PREEMPT_RT 对照保留为发布后的独立 Gate。
...
阶段 2～4（旧执行编号 P1～P3）的工作包、线程/fd 所有权和证据解释见归档的
[P1–P3 详细执行记录](../archive/P1_P3_EXECUTION_PLAN.md)。它和本路线都不负责近期排期。

## 1. 路线总览

下图是长期能力的参考顺序，不是 V1 关闭后的自动任务选择：
...
| RT-Lab | 普通内核基线、实时编程与 PREEMPT_RT 安全对照 | RT0–RT4/RT6/RT7 已收口；RT5 blocked | 是 |
| 5 | EtherCAT MainDevice + simple I/O SubDevice | V1 后候选；尚未选为下一 Gate | 否；使用 ThinkPad，但需要从站 |
| 6 | Modbus TCP 学习实验 | 已有独立 demo；未接入 Runtime | 推荐，但可先本机 |
...
历史工作包见 [P1–P3 详细执行记录](../archive/P1_P3_EXECUTION_PLAN.md) 的 P1 部分；
...
**未关闭**：Orange Pi clean commit 复跑与 B4；stock 无 CAN 时 `rcrd` 不 active 属于已知
平台边界，不用 FakeCan service 掩盖。

## 6. 阶段 3：ThinkPad 证据基线

状态：**Implementation complete / clean evidence re-capture open**
历史工作包见 [P1–P3 详细执行记录](../archive/P1_P3_EXECUTION_PLAN.md) 的 P2 部分；
...
本地 `342fb0d` 当时得到 19/19 故障场景和 12/12 benchmark 格（程序现为 22 场，未当作
该提交的追加结论）。sanitizer 重跑 0 字节报告
缺陷已在脚本侧修复（`mktemp -d` + 同目录原子 rename + `秒.PID` 文件名）；连续两次
ASan/UBSan=`pass`、TSan=`unsupported`。正式 clean-commit 证据与 rcrd 重复启停归入
V1 发布 Gate R2。运行产物按用户决定可不提交。

## 7. 阶段 4：Orange Pi 部署

历史工作包见 [P1–P3 详细执行记录](../archive/P1_P3_EXECUTION_PLAN.md) 的 P3 部分；
...
- physical CAN：只有需要位时序、错误计数、bus-off 和波形证据时采购；若选择该链路，
  当前候选是 [STM32F103 CAN + SG90 双位置实验](../archive/STM32_CAN_SG90_EXPERIMENT.md)，先过
...
6. **RT1**：runner 已落地；**60s smoke 10/10 已审阅**（dirty Release，governor enforced；
   见 `evidence/portfolio/orangepi_rt1_smoke_20260805.md`）。当时记录的后续动作是提交后 clean
...
V1 关闭后，EtherCAT、物理 CAN 或其它独立 Gate 的优先级需重新评审。Modbus TCP 可以保留
