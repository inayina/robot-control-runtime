...
当前 Linux Core
      ↓
CAN V1 codec + vcan node simulator
      ↓
rcrd + epoll + signalfd/eventfd
      ↓
ThinkPad 故障矩阵与 benchmark
      ↓
Orange Pi SSH + systemd + ARM 实测
      ↓
...
退出条件：重复启动/停止无 fd 泄漏；SIGTERM 有界退出；节点离线进入可见 Fault；
systemd 所需进程行为在 ThinkPad 上可验证。
