...
Runtime-connected CAN Health + 冻结结果 schema / 原子 JSON+CSV 写入；
IPC 和 Qt UI 尚未实现**
...
- [Actuator 01 Commissioning Profile 详细设计](ACTUATOR_COMMISSIONING_PROFILE_PLAN.md)
...
robot-control-runtime/
├─ linux/
│  ├─ include/rcr/workbench/        # 无 Qt 的 test/diagnostic 核心
│  ├─ src/workbench/
│  ├─ tests/                        # evaluator、result、session 测试
│  └─ tools/
│     └─ qt_device_workbench/       # OPTIONAL Qt presentation/application
├─ protocol/                        # 仍只放冻结的线级合同
└─ docs/
   └─ DEVICE_TEST_DIAGNOSTIC_WORKBENCH_DEVELOPMENT_PLAN.md
```
...
以下是后续阶段候选，不表示现在创建；出现重复职责时优先合并到现有模型：

```text
linux/include/rcr/workbench/
  diagnostic_event.hpp
  test_case.hpp
  test_result.hpp
  test_runner.hpp
  result_writer.hpp
...
linux/tools/qt_device_workbench/
  ...
  main.cpp
  main_window.hpp/.cpp
  workbench_controller.hpp/.cpp
  presentation_models.hpp/.cpp
...
### Phase 4 — Optional Qt6 Workbench
...
Phase 3 实现到此停止，不自动进入 Phase 4。下一阶段应由用户确认后再做
