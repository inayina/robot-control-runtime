...
void require_vcan_or_skip() {
  ...
  // sysfs 可见不等于当前进程能打开 PF_CAN（沙箱/权限不足时应 Skip，而非假
  // FAIL）。
...
bool run_ip_link(const char *iface, const char *action) {
  ...
  if (pid == 0) {
    ::execlp("ip", "ip", "link", "set", "dev", iface, action,
...
void require_iface_down_authorization_or_skip() {
  ...
  if (!env_flag_enabled("RCR_ALLOW_IFACE_DOWN")) {
    RCR_SKIP("set RCR_ALLOW_IFACE_DOWN=1 to authorize vcan link-down fault "
             "injection");
  }
}
...
RCR_TEST(DaemonOnlineHeartbeatAndBoundedStop) {
  ...
  ChildProcess sim;
  RCR_REQUIRE(sim.start(find_node_sim(), {"--can", "vcan0", "--node-id", "1",
                                          "--heartbeat-ms", "50"}));
  ...
  const bool interlock =
      wait_until([&] { return daemon.snapshot().runtime.interlock_ready; },
                 std::chrono::milliseconds{2000});
  ...
  const bool applied = wait_until(
      [&] {
        const auto snap = daemon.snapshot();
        return snap.io.frames_sent >= 1 && !snap.runtime.output_ack_pending &&
               snap.runtime.last_ack_sequence == 1 &&
               snap.runtime.last_ack_result ==
...
RCR_TEST(DaemonVcanInterfaceDownPropagatesIoError) {
  ...
  // 并映射为 WorkerFailure。默认 Skip；需 RCR_ALLOW_IFACE_DOWN=1 且具备
  // CAP_NET_ADMIN。
  ...
  // stop_reason 必须在 wait_and_stop() 之前采样：stop 会 io_.reset()，之后
  // snapshot().io 变回默认 None，会把已经成功的 IO_ERROR 误判成失败。
  ...
  RCR_EXPECT(run_ip_link("vcan0", "up") ||
             rcr::probe_can_interface("vcan0") ==
                 rcr::CanInterfaceStatus::Available);
}
