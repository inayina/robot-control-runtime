#!/usr/bin/env bash
# ThinkPad 调度/负载矩阵：OTHER/FIFO × idle/stress-ng × 1/5/10 ms。
# 不修改 governor/limits；先探测并记录 unsupported/permission_denied。
set -euo pipefail
...
OUT_DIR="${ROOT}/evidence/thinkpad_baseline/${STAMP}"
FIFO_PRI="${RCR_BENCH_FIFO_PRIORITY:-10}"

if [[ ! -x "${BENCH}" ]]; then
  echo "error: build rcr_benchmark first" >&2
  exit 1
fi
if [[ -e "${OUT_DIR}" ]]; then
...
echo "See SUMMARY.txt for per-cell results. Not hard realtime; ThinkPad baseline only."
if [[ "${failed}" -ne 0 ]]; then
  exit 1
fi
