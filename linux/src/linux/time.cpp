...
Result<std::int64_t> monotonic_now_ns() noexcept {
  timespec now{};
  // clock_gettime 失败时 errno
  // 只在当前线程有效，立即复制到诊断字符串；不能退化到
...
Result<std::int64_t> realtime_now_ns() noexcept {
  timespec now{};
  if (::clock_gettime(CLOCK_REALTIME, &now) != 0) {
    return Error{Errc::IoError, std::string("clock_gettime(CLOCK_REALTIME): ") +
                                    std::strerror(errno)};
  }
  return timespec_to_ns(now);
}
...
timespec ns_to_timespec(std::int64_t nanoseconds) noexcept {
  timespec value{};
  // POSIX timespec
  // 把秒和不足一秒的纳秒分开；输入合同为非负，因此余数天然落在规范范围。
