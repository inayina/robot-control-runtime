...
LinuxRuntime::LinuxRuntime(RuntimeConfig config)
    : config_(config), command_watchdog_(config.command_timeout),
      trace_(config.trace_capacity), scheduler_(config.scheduler),
      test_throw_on_tick_(config.test_throw_on_tick) {}

LinuxRuntime::~LinuxRuntime() { stop(); }

Result<void> LinuxRuntime::start() {
  // timeout 在构造 watchdog 时保存，但仍在真正创建线程前校验，使配置错误以
  // Result 返回，而不是让一个永远立即过期的监督器进入运行状态。
  if (config_.command_timeout.count() <= 0 ||
      config_.output_ack_timeout.count() <= 0) {
    return Error{Errc::InvalidArgument,
                 "command watchdog and output ACK timeouts must be positive"};
  }
  return scheduler_.start([this](const SchedulerTick &tick) { on_tick(tick); });
}

void LinuxRuntime::stop() {
  // 必须先停止并 join worker，再取得 state_mutex_。若反过来持锁等待 join，而
  // worker 正在 on_tick 中等待同一把锁，会形成确定性死锁。
...
TransitionResult LinuxRuntime::handle(RuntimeEvent event) {
  // 取时失败不阻止纯状态事件，但 trace 使用 0
  // 明确表示“没有有效时间戳”。状态迁移本身 仍在 state_mutex_ 下串行，避免
  // Application 与周期 timeout 同时改变模式。
  const auto now = monotonic_now_ns();
  std::lock_guard lock(state_mutex_);
  if (event == RuntimeEvent::FaultDetected) {
    // FaultDetected 不能再通过通用事件 API 单独投递，否则 fault code
    // 与模式迁移会重新 变成两阶段
    // TOCTOU。纯状态机测试仍可直接测试该事件，Runtime 调用者必须 raise_fault。
    return TransitionResult{
        false, state_machine_.mode(), state_machine_.mode(),
        "use raise_fault(code) for atomic fault escalation"};
  }
...
TransitionResult LinuxRuntime::raise_fault(FaultCode code) {
  const auto now = monotonic_now_ns();
  std::lock_guard lock(state_mutex_);
  if (code == FaultCode::None) {
    return TransitionResult{false, state_machine_.mode(), state_machine_.mode(),
                            "raise_fault requires a non-NONE fault code"};
  }

  // fault 字段、模式迁移和所有输出许可资源属于一个提交事务。尤其不能在
  // set_fault 与 FaultDetected 之间释放锁，否则发布线程可在 mode 仍为 Active
  // 时接受一条命令。
  state_machine_.set_fault(code);
  const TransitionResult transition =
      state_machine_.handle(RuntimeEvent::FaultDetected);
  clear_output_path_locked();
  trace_.record(TraceEvent{now ? now.value() : 0, TraceKind::FaultRaised,
...
void LinuxRuntime::set_interlock_ready(bool ready) {
  // 联锁信息和由此产生的状态迁移在同一锁区间完成；不能先写 ready、稍后再清
  // mailbox， 否则消费线程可能在两步之间取走最后一条输出。
  const auto now = monotonic_now_ns();
  std::lock_guard lock(state_mutex_);
  const RuntimeMode before = state_machine_.mode();
...
Result<void>
LinuxRuntime::publish_output_command(const OutputCommand &command) {
  // deadline 与当前时间必须来自同一个 CLOCK_MONOTONIC
  // 域。先取时再加锁可缩短临界区；
  ...
  const auto reject = [this, &command,
                       now_ns = now.value()](const char *message) {
  ...
  if (command.session_id == 0 || command.sequence == 0 || command.mask == 0) {
    return reject(
        "output command requires nonzero session, sequence, and mask");
  }
  ...
  if (!active_session_id_.has_value()) {
    // 第一条合法命令定义本次 Active
    // 的应用会话；绑定必须晚于所有基本合法性检查， 否则一条 mask=0
    // 或已过期的坏命令会占住 session，拒绝后续正常命令。
  ...
  // 状态检查、序号提交、邮箱发布和 watchdog kick 位于同一状态锁区间，避免 Hold
  // 后 仍有命令从竞态窗口穿过软件门控。
...
std::optional<OutputCommand> LinuxRuntime::try_consume_output_command() {
  // 发布时检查 deadline 仍不足以保证消费时新鲜：latest-wins 槽位可能因 I/O
  // 忙而等待， 所以在真正交给输出路径前必须重新取单调时间并复查。
  const auto now = monotonic_now_ns();
  if (!now) {
    return std::nullopt;
  }
  ...
  if (output_ack_pending_) {
    // 已发送命令的执行结果尚未闭合。新目标继续在 mailbox 中
    // latest-wins，不能形成 多笔在途命令，否则单个 last_sent 状态无法判断乱序
    // ACK 属于哪条命令。
    return std::nullopt;
  }
...
bool LinuxRuntime::output_command_sendable(const OutputCommand &command) const {
  ...
  return scheduler_.running() && state_machine_.can_accept_output() &&
         !output_ack_pending_ && active_session_id_.has_value() &&
         *active_session_id_ == command.session_id &&
         command.deadline_ns > now.value();
}

Result<void> LinuxRuntime::note_output_command_sent(std::uint16_t session_id,
                                                    std::uint16_t sequence,
                                                    std::int64_t sent_time_ns) {
  std::lock_guard lock(state_mutex_);
  if (session_id == 0 || sequence == 0 || sent_time_ns <= 0) {
    return Error{
        Errc::InvalidArgument,
        "sent output requires nonzero wire session, sequence, and timestamp"};
  }
  if (!scheduler_.running() || !state_machine_.can_accept_output() ||
      !active_session_id_.has_value() ||
      static_cast<std::uint16_t>(*active_session_id_) != session_id) {
    return Error{Errc::Rejected,
                 "runtime closed while output command was being sent"};
  }
  if (output_ack_pending_) {
    return Error{Errc::Busy, "an output command ACK is already pending"};
  }

  // Socket 写成功才建立在途事实；publish/consume 都不能冒充节点已经收到命令。
  output_ack_pending_ = true;
  last_sent_session_ = session_id;
  last_sent_sequence_ = sequence;
  last_sent_time_ns_ = sent_time_ns;
  trace_.record(TraceEvent{sent_time_ns, TraceKind::OutputCommandSent,
                           static_cast<std::int64_t>(sequence),
                           static_cast<std::int64_t>(session_id)});
  return Result<void>::success();
}

void LinuxRuntime::observe_output_status(std::uint16_t session_id,
                                         std::uint16_t sequence,
                                         can_v1::OutputResult result,
                                         std::int64_t ack_time_ns) {
  std::lock_guard lock(state_mutex_);
  last_ack_session_ = session_id;
  last_ack_sequence_ = sequence;
  last_ack_result_ = result;
  last_ack_time_ns_ = ack_time_ns;
  trace_.record(TraceEvent{ack_time_ns, TraceKind::OutputAckObserved,
                           static_cast<std::int64_t>(sequence),
                           static_cast<std::int64_t>(result)});

  const bool matches = output_ack_pending_ &&
                       session_id == last_sent_session_ &&
  ...
  ++unexpected_ack_count_;
  trace_.record(TraceEvent{ack_time_ns, TraceKind::OutputAckUnexpected,
                           static_cast<std::int64_t>(sequence),
                           static_cast<std::int64_t>(result)});
  // 拒绝 ACK 或不属于当前在途命令的 ACK
  // 都不解锁下一次发送；若正确确认一直缺失， 周期路径会在 ACK timeout 后进入
  // Hold。这里不自动重试，也不把旧 ACK 当成功。
}
...
RuntimeSnapshot LinuxRuntime::snapshot() const {
  ...
  RuntimeSnapshot value{};
  {
    ...
    value.output_ack_pending = output_ack_pending_;
    value.last_sent_session = last_sent_session_;
    value.last_sent_sequence = last_sent_sequence_;
    value.last_sent_time_ns = last_sent_time_ns_;
    value.last_ack_session = last_ack_session_;
    value.last_ack_sequence = last_ack_sequence_;
    value.last_ack_result = last_ack_result_;
    value.last_ack_time_ns = last_ack_time_ns_;
    value.ack_timeout_count = ack_timeout_count_;
    value.unexpected_ack_count = unexpected_ack_count_;
  }
...
std::vector<TraceEvent> LinuxRuntime::trace_snapshot() const {
...
void LinuxRuntime::on_tick(const SchedulerTick &tick) {
  ...
  // ACK timeout 与 command watchdog、publish/kick 共用
  // state_mutex_，使两类超时和新命令 只有一个全序。ACK timeout 先判定，避免同一
  // tick 同时归因成两种 Hold。
  // 这里仍需要组合级串行化，保证“检查到超时并清输出”和“接受新命令并
  // kick”有唯一顺序。
  {
    std::lock_guard lock(state_mutex_);
    if (state_machine_.can_accept_output() && output_ack_pending_ &&
        tick.actual_ns >= last_sent_time_ns_ &&
        tick.actual_ns - last_sent_time_ns_ >=
            config_.output_ack_timeout.count()) {
      ++ack_timeout_count_;
    ...
    if (state_machine_.can_accept_output()) {
      ...
      if (check.newly_expired) {
        trace_.record(TraceEvent{tick.actual_ns, TraceKind::WatchdogExpired,
                                 check.age_ns, 0});
        const TransitionResult transition =
  ...
  // 监督钩子在锁外运行，以便 NodeSupervisor 回调 handle/raise_fault
  // 时不会自死锁。
...
void LinuxRuntime::clear_output_path_locked() {
  // 调用者必须已持有 state_mutex_。顺序先 disarm
  // 再清数据，防止未来无锁观察者在清理 过程中把旧 last_kick
  // 当成仍受监督的活动会话；clear 不重置历史诊断计数。
