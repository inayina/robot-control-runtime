...
/// 读取
/// CLOCK_MONOTONIC，返回自某个未指定起点以来的纳秒数；只应用于同机同一启动期内
/// 的差值/排序，不受系统校时和时区变化影响，也不能作为跨机器线协议时间戳。
[[nodiscard]] Result<std::int64_t> monotonic_now_ns() noexcept;

/// 读取 CLOCK_REALTIME 的 Unix 纪元纳秒，只用于结果文件/日志的墙钟标注。
/// 禁止用于 deadline、watchdog、lease 或任何跨重启可比较的控制时间。
[[nodiscard]] Result<std::int64_t> realtime_now_ns() noexcept;
